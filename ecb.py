...
def cifrar(nombre,ciphertext_img):
	# Guardar la imagen cifrada en un nuevo archivo
	with open(nombre[:nombre.index('.')]+'_eECB.bmp', 'wb') as f:
		f.write(ciphertext_img)



def inicio(nombre,op):
	...
	if op==1:
		...
		print(nombre)
		sal= open(nombre[:nombre.index('.')]+'_dECB.bmp', 'wb')
		sal.write(decrypted_img)
